  ...
  useEffect(() => {
    ...
    const fetchProductData = async () => {
      try {
        ...
        const mainImageIndex = productRes.data.imageUrl.findIndex(
          (img) => img === productRes.data.mainImage
        );
        setSelectedImage(mainImageIndex >= 0 ? mainImageIndex : 0);

        // Initialize first size and color as selected
        const sizes = productRes.data.sizes
          ?.split(",")
          .map((s) => s.trim())
          .filter(Boolean);
        const colors = productRes.data.colors
          ?.split(",")
          .map((c) => c.trim())
          .filter(Boolean);
        ...
        try {
          const reviewsRes = await reviewsApi.getReviewsByProduct(
            productRes.data.id
          );
        ...
        try {
          const relatedRes = await productsApi.getRelatedProducts(
            productRes.data.id
          );
          ...
          console.warn(
            "Related products endpoint not available:",
            relatedError
          );
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="flex space-x-2 overflow-x-auto">
              {product.imageUrl && product.imageUrl.length > 0 ? (
                ...
                <button className="w-20 h-20 relative rounded-md overflow-hidden border-2 border-blue-500">
                  <Image
          ...
          <div className="flex flex-col">
              ...
              {product.dealPrice ? (
                <div className="flex items-baseline gap-2">
                  ...
                  <Badge variant="secondary">
                    SAVE{" "}
                    {Math.round(
                      ((product.price - product.dealPrice) / product.price) *
                        100
                    )}
                    %
                  </Badge>
                  ...
                  {product.sizes.split(",").map((size) => {
                    ...
                    return (
                      <Button
                        key={trimmedSize}
                        variant={
                          selectedSize === trimmedSize ? "default" : "outline"
                        }
                        className="px-4 py-2 bg-[#F7F7F7] dark:bg-[#263238] border-[#455A64] text-[#263238] dark:text-white dark:border-[#B0BEC5] hover:bg-[#263238] dark:hover:bg-[#F7F7F7] hover:text-white dark:hover:text-[#263238] transition-colors"
                  ...
                  {product.colors.split(",").map((color) => {
                    ...
                    return (
                      <Button
                        key={trimmedColor}
                        variant={
                          selectedColor === trimmedColor ? "default" : "outline"
                        }
                        className="px-4 py-2 bg-[#F7F7F7] dark:bg-[#263238] border-[#455A64] text-[#263238] dark:text-white dark:border-[#B0BEC5] hover:bg-[#263238] dark:hover:bg-[#F7F7F7] hover:text-white dark:hover:text-[#263238] transition-colors"
            ...
            <div className="flex items-center gap-4 mb-6">
              <div className="flex items-center border border-gray-300 dark:border-gray-600 rounded-md">
                <Button
                  variant="ghost"
                  className="bg-[#F7F7F7] dark:bg-[#263238] border-[#455A64] text-[#263238] dark:text-white dark:border-[#B0BEC5] hover:bg-[#263238] dark:hover:bg-[#F7F7F7] hover:text-white dark:hover:text-[#263238] transition-colors"
              ...
              <Button
                size="lg"
                className="flex-1 bg-[#F7F7F7] dark:bg-[#263238] border-[#455A64] text-[#263238] dark:text-white dark:border-[#B0BEC5] hover:bg-[#263238] dark:hover:bg-[#F7F7F7] hover:text-white dark:hover:text-[#263238] transition-colors"
              >
                Add to Cart
              </Button>
            </div>
                      ...
                      {product.activeDeal && (
                        <div className="bg-red-50 dark:bg-red-900/20 p-3 rounded border-l-4 border-red-500">
                          ...
                          <p className="text-red-600 dark:text-red-300 text-sm">
                            {product.activeDeal.description ||
                              `${product.activeDeal.discount}% discount available`}
                          </p>
                        </div>
                      )}
                    </div>
